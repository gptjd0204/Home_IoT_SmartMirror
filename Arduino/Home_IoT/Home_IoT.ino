...
void connectMQTT() {
  ...
  // $aws/things/MyMKR2/shadow/update/delta 구독
  mqttClient.subscribe("$aws/things/MyMKR2/shadow/update/delta");
}
...
void getDeviceStatus(char* payload) {
  ...
  // $aws/things/MyMKR2/shadow/update topic으로 payload 전송
  sprintf(payload,"{\"state\":{\"reported\":{\"SERVO_STATE\":\"%s\"}}}",servoState);
}

// MQTT Client에 메시지를 보낸다.
void sendMessage(char* payload) {
  char TOPIC_NAME[]= "$aws/things/MyMKR2/shadow/update";
...
void onMessageReceived(int messageSize) {
  ...
  // AWS로부터 메시지를 받을 때 SERVO_STATE가 ON이면 실내등 켜기, OFF이면 실내등 끄기
